# -*- coding: utf-8 -*-
"""
光伏支架线模生成器 V1.2.30 — 界面骨架（MVP M1，2026-08-12 第三十三版）

运行方式：
    python pv_support_gui.py

V1.2.30 改动（消除导入警告）：
    1. 槽钢改为 DB 行（按库名 [8 引用，同角钢做法）；
    2. 删除截面行后多余的第二行（样例中本无此行），
       "第14行参数错误"警告应消失；
    3. 方位角保持：斜梁 0°、檩条 110°、立柱 180°、斜撑 90°。
...
class PvSupportApp:
    def __init__(self, root):
        ...
        self.set_status("就绪 V1.2.30：截面行格式修正，导入警告应全部消除")
